(function () {
	...
	RS.prototype.createScale = function (resize) {
		...
		//this.step = Number(this.step).toFixed(3);		
		this.step = Math.floor(this.step * 1000) / 1000;
		...
		for (var i = 0, iLen = this.conf.values.length; i < iLen; i++) {
			...
			if (!this.conf.labels) {
				...
				ins.innerHTML = (this.conf.values[i].indexOf('Jan') == 0) ? this.conf.values[i].replace('Jan','').trim() : '';
			}

			ins.style.marginLeft = (ins.clientWidth / 2) * - 1 + 'px';
			if((parseInt(ins.style.marginLeft) == 0)){
				ins.style.marginLeft = "-10px";
			}
		}
	...
	RS.prototype.setValues = function (start, end) {
		...
		this.pointerL.style.left = (this.values[activePointer] * this.step - (this.pointerWidth / 2)) -10 + 'px';
		...
		if (this.conf.range) {
			...
			this.pointerR.style.left = (this.values.end * this.step - (this.pointerWidth / 2)) -12 + 'px';
		}
		else {
			if (this.conf.tooltip)
	...
	RS.prototype.onResize = function () {
		this.sliderLeft = this.slider.getBoundingClientRect().left;
		this.sliderWidth = (this.slider.getBoundingClientRect()) ? this.slider.getBoundingClientRect().width : this.slider.clientWidth;
