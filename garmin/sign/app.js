...
const vapidKeys = {
	publicKey: 'BPbx7hccyKAGl_M9b1qZ4ZBip0jaafGAaxUzxhlAzh8kWWqNp75vblrnNCMeOCVIbzAiXFSRZUIlwKR4679woh4',
	privateKey: 'bLHY7CA0kXQD9b5JQIlYRYIetTVkXRpWqWzSI1Wwh24'
}
...
app.post('/garmin/requestToken', (req, res)=> {
	...
	axios.post( request.url, request.body, { headers: authHeader })
	  .then(response => {
		...
		//var userAuthUrl = "https://connect.garmin.com/oauthConfirm?oauth_token=" + oauthToken + "&oauth_callback=https://tslwellnessapp.mobilous.com/hnw/index.html";
...
app.post('/garmin/accessToken', (req, res)=> {	
	...
	axios.post( request.url, request.body, { headers: authHeader })
	  .then(response => {
		...
		//console.log(userToken, '.....access Token.... ', userSecret);	console.log("");
		//{ "usertoken": "97db7c23-542d-48e2-be82-5eb6c120b3e9", "usersecret": "1ysK0a42cgNSHp0s6ObDbkzwkq7BJfuW2OG" }
		if(registeredusers){
...
var setResponseEnd = function(resp, userId) {
	...
	//activityRecords = activityRecords.filter(activity => activity.userAccess !== userId);
...
function sendSummaryDataToAll(records) {
	console.log("sendSummaryDataToAll >>", records);
...
var setUserActivityData = function(arrActivity, type) {
	...
	for(var i=0; i< filteredArr.length; i++){
		var myObj = {};
		myObj['stepswalked'] = (filteredArr[i]['steps']) ? filteredArr[i]['steps'] : 0;		
		myObj['durationInSeconds'] = (filteredArr[i]['activeTimeInSeconds']) ? filteredArr[i]['activeTimeInSeconds'] : filteredArr[i]['durationInSeconds'];
		myObj['distanceInMeters'] = filteredArr[i]['distanceInMeters'];
		myObj['speedms'] = (filteredArr[i]['averageSpeedInMetersPerSecond']) ? filteredArr[i]['averageSpeedInMetersPerSecond'] : 0;
		myObj['calories'] = filteredArr[i]['activeKilocalories'];
		myObj['startTimeInSeconds'] = filteredArr[i]['startTimeInSeconds'];
		myObj['userAccess'] = filteredArr[i]['userAccessToken'];
		myObj['userId'] = filteredArr[i]['userId'];
